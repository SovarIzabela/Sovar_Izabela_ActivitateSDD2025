...
void adaugaFarmacieInVector(struct Farmacie** farmacie, int* nrFarmacii, struct Farmacie farmacieNoua) {
	...
	struct Farmacie* aux = (struct Farmacie*)malloc(sizeof(struct Farmacie) * (*nrFarmacii + 1));

	
	for (int i = 0; i < (*nrFarmacii); i++) {
		aux[i].id = (*farmacie)[i].id;
		aux[i].nume = (char*)malloc(strlen((*farmacie)[i].nume) + 1);
		strcpy(aux[i].nume, (*farmacie)[i].nume);
		aux[i].numarMedicamente = (*farmacie)[i].numarMedicamente;
		aux[i].preturiMedicamente = (float*)malloc(sizeof(float) * (*farmacie)[i].numarMedicamente);
		for (int j = 0; j < (*farmacie)[i].numarMedicamente; j++) {
			aux[i].preturiMedicamente[j] = (*farmacie)[i].preturiMedicamente[j];
		}
	}

	
	aux[*nrFarmacii].id = farmacieNoua.id;
	aux[*nrFarmacii].nume = (char*)malloc(strlen(farmacieNoua.nume) + 1);
	strcpy(aux[*nrFarmacii].nume, farmacieNoua.nume);
	aux[*nrFarmacii].numarMedicamente = farmacieNoua.numarMedicamente;
	aux[*nrFarmacii].preturiMedicamente = (float*)malloc(sizeof(float) * farmacieNoua.numarMedicamente);
	for (int i = 0; i < farmacieNoua.numarMedicamente; i++) {
		aux[*nrFarmacii].preturiMedicamente[i] = farmacieNoua.preturiMedicamente[i];
	}


	for (int i = 0; i < *nrFarmacii; i++) {
		free((*farmacie)[i].nume);
		free((*farmacie)[i].preturiMedicamente);
	}
	free(*farmacie);
...
struct Farmacie* citireVectorFarmacieDinFisier(char* file, int* numarFarmacii) {

	FILE* f = fopen(file, "r");
	struct Farmacie* farmacie = NULL;
	*numarFarmacii = 0;

	while(!feof(f)) {
		adaugaFarmacieInVector(&farmacie, numarFarmacii, citireFarmacieDinFisier(f));
	}
...
void main(){
	...
	int nrFarmacii = 0;
	struct Farmacie* farmacieFisier = citireVectorFarmacieDinFisier("farmacie.txt", &nrFarmacii);
	printf("------------Vector farmacie din fisier------------\n");
	afisareVector(farmacieFisier, nrFarmacii);
	dezalocareV(&farmacieFisier, &nrFarmacii);
